    ...
    return (
      <List>
        <List.EmptyView
          ...
          actions={
            <ActionPanel>
              <Action.OpenInBrowser title="Download TidyPorts" url={DOWNLOAD_URL} />
...
function ServerItem({ listener, onChanged }: { listener: Listener; onChanged: () => void }) {
  const url = `http://localhost:${listener.port}`;
  const who = provenance(listener);
  const project = listener.gitRoot ? listener.gitRoot.split("/").pop() : undefined;

  const accessories: List.Item.Accessory[] = [];
  if (listener.branch) accessories.push({ tag: { value: listener.branch, color: Color.SecondaryText } });
  ...
  if (listener.exposed && listener.exposedAddr && listener.exposedAddr !== "127.0.0.1") {
    accessories.push({ icon: { source: Icon.Globe, tintColor: Color.Orange }, tooltip: `Reachable on ${listener.exposedAddr}` });
  }
  ...
  return (
    <List.Item
      ...
      keywords={[listener.comm, project, listener.branch, listener.agent, listener.surface].filter(Boolean) as string[]}
      ...
      actions={
        <ActionPanel>
          ...
          <Action
            ...
            onAction={async () => {
              ...
              const ok = await confirmAlert({
                ...
                primaryAction: { title: "Stop Server", style: Alert.ActionStyle.Destructive },
              });
              if (!ok) return;
              const toast = await showToast({ style: Toast.Style.Animated, title: `Stopping :${listener.port}…` });
              try {
          ...
          <Action title="Refresh" icon={Icon.ArrowClockwise} shortcut={{ modifiers: ["cmd"], key: "r" }} onAction={onChanged} />
        </ActionPanel>
      }
    />
  );
